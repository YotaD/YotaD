#!/usr/bin/env python
"""
twitter.py - Yota Twitter Module
...
def twitter(m5, input):
   arg = input.group(2)
   if not arg:
      return m5.reply("Give me a link, a username, or a tweet id")
   ...
   if arg.isdigit():
      ...
      m5.say(user_tweet(arg))
   elif r_link.match(arg):
      username = arg.split('/')[3]
      tweet = read_tweet(arg)
      m5.say(format(tweet, username))
   else: m5.reply("Give me a link, a username, or a tweet id")
