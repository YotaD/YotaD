#!/usr/bin/env python
'''
mcstatus.py - Yota MinecraftStatus Module
...
def version(m5, input):    
    try:
        query = MinecraftQuery(server, port)
        status = query.get_rules()
        m5.say("Minecraft version: %s Software version: %s" % (status["version"],
                                                             str(status["software"])))
    except:
        m5.say("Failed to query server - try again later")
...
def plugins(m5, input):
    '''!plugins - Returns a list of plugins'''   
    try:
        query = MinecraftQuery(server, port)
        status = query.get_rules()
        m5.say("Plugins: [ %s ]" % ", ".join(status["plugins"]))
    except:
        m5.say("Failed to query server - try again later")
