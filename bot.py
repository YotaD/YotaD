...
class m5(irc.Bot): 
   ...
   def call(self, func, origin, m5, input): 
      try: func(m5, input)
      except Exception, e: 
         self.error(origin)
         ...
         for regexp, funcs in items: 
            for func in funcs: 
               ...
               if match: 
                  ...
                  m5 = self.wrapped(origin, text, match)
                  input = self.input(origin, text, bytes, match, event, args)

                  if func.thread: 
                     targs = (func, origin, m5, input)
                     t = threading.Thread(target=self.call, args=targs)
                     t.start()
                  else: self.call(func, origin, m5, input)
